...
public class AiModelApiController implements AiModelApi {
    ...
    private final AiModelApiDelegate delegate;

    public AiModelApiController(@Autowired(required = false) AiModelApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new AiModelApiDelegate() {});
    }

    @Override
    public AiModelApiDelegate getDelegate() {
        return delegate;
    }
