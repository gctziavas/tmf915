...
public class RuleApiController implements RuleApi {
    ...
    private final RuleApiDelegate delegate;

    public RuleApiController(@Autowired(required = false) RuleApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new RuleApiDelegate() {});
    }

    @Override
    public RuleApiDelegate getDelegate() {
        return delegate;
    }
