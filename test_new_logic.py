...
# Scenario 1) Failing deployment (MinIO missing, wrong model/version)
print("--- SCENARIO 1: FAILING DEPLOYMENT ---")
# Find a dummy spec first:
specs = requests.get(aimodel_spec_url, headers=headers).json()
spec_fail = next((s for s in specs if 'champion-pipeline-iris' in s['name']), specs[0])
print(f"Using Spec: {spec_fail['id']}")

model_id_fail = create_and_deploy_model("failing-model-test", spec_fail['id'])
if model_id_fail:
    print(f"Created model {model_id_fail}. Waiting 5 seconds for background deploy...")
    time.sleep(5)
    model_data = check_model_state(model_id_fail)
    print("State after failure:", model_data.get('state'))
    print("Notes directly on model:", json.dumps(model_data.get('note', []), indent=2))
    
    spec_data = check_spec_state(spec_fail['id'])
